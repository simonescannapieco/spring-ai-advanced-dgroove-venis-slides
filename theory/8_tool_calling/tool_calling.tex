...
{\footnotesize
    \begin{minipage}[t]{\textwidth}
        \begin{itemize}[leftmargin=10pt,align=right]
            \item[\alert{\faExclamationTriangle}] \textbf{Limitazioni} (vincoli di risoluzione dei tipi a \textit{runtime}):
            \begin{itemize}[leftmargin=10pt,align=right]
                \onslide<2->\item[\alert{\faArrowCircleRight}] Tipi primitivi
                \onslide<3->\item[\alert{\faArrowCircleRight}] \texttt{Optional}
                \onslide<4->\item[\alert{\faArrowCircleRight}] Collezioni (\texttt{List}, \texttt{Map}, \texttt{Array}, \texttt{Set})
                \onslide<5->\item[\alert{\faArrowCircleRight}] Tipi asincroni e reattivi
            \end{itemize}
        \end{itemize}
    \end{minipage}
}
